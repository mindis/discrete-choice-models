...
##                          outputs
################################################################################
EstimOpt$strModelName <- "Equality Constrained Latent Class - Multinomial Logit Model -- ECLC-MNL"
EstimOpt$strOutputFile <- "output.eclc.mnl.coral"
...
################################################################################
EstimOpt$strP_fixed <- c("small", "large", "oil", "fish", "hab", "cost")
...
##  See ?randtoolbox
################################################################################
EstimOpt$bMakeDraws <- FALSE
EstimOpt$bCorrelation <- FALSE
EstimOpt$strDrawType <- "sobol" 
EstimOpt$iR <- 50L
EstimOpt$iD <- 0L
...
################################################################################
EstimOpt$bEqualityConstrained <- TRUE
EstimOpt$bDiscreteMixture <- TRUE
EstimOpt$mConstraints <- NULL 
EstimOpt$lsP_constrained <- list(size = c("small", "large"),
                                 industry = c("oil", "gas"),
                                 hab = c("hab"),
                                 cost = c("cost"))
